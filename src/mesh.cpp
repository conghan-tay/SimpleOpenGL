...
void ComputeNormals(Mesh &mesh)
{
	...
	// For each vertex
	for (unsigned int i = 0; i < mesh.vertexBuffer.size(); ++i)
	{
		mesh.vertexBuffer[i].nrm = Vec3(0.0f);

		unsigned int count = 0; // number of triangles with this vertex

		// loop thru each triangle by doing j+=3 and find if triangle
		// has vertex
		for (unsigned int j = 0; j < mesh.indexBuffer.size(); j+=3)
		{

			Vec3 c_normal = Vec3(0.0f);

			if (mesh.indexBuffer[j] == i)
			{
				++count;
				Vec3 first, second;
				first = mesh.vertexBuffer[mesh.indexBuffer[j + 1]].pos - mesh.vertexBuffer[mesh.indexBuffer[j]].pos;
				second = mesh.vertexBuffer[mesh.indexBuffer[j + 2]].pos - mesh.vertexBuffer[mesh.indexBuffer[j]].pos;

				c_normal = Cross(first, second);

				// make sure to normalize before adding
				c_normal = glm::normalize(c_normal);
			}
			else if (mesh.indexBuffer[j + 1] == i)
			{
				++count;
				Vec3 first, second;
				first = mesh.vertexBuffer[mesh.indexBuffer[j + 2]].pos - mesh.vertexBuffer[mesh.indexBuffer[j + 1]].pos;
				second = mesh.vertexBuffer[mesh.indexBuffer[j]].pos - mesh.vertexBuffer[mesh.indexBuffer[j + 1]].pos;
			...
			else if (mesh.indexBuffer[j + 2] == i)
			{
				
				++count;
				Vec3 first, second;
				first = mesh.vertexBuffer[mesh.indexBuffer[j]].pos - mesh.vertexBuffer[mesh.indexBuffer[j + 2]].pos;
				second = mesh.vertexBuffer[mesh.indexBuffer[j + 1]].pos - mesh.vertexBuffer[mesh.indexBuffer[j + 2]].pos;
			...
			// add current normal to accumulated normal
			mesh.vertexBuffer[i].nrm += c_normal;
		}

		// Average and normalize 
		mesh.vertexBuffer[i].nrm /= (float) count;
		mesh.vertexBuffer[i].nrm = glm::normalize(mesh.vertexBuffer[i].nrm);
	}
}


bool maxMinY(Vertex l, Vertex r)
{
	return l.pos.y < r.pos.y;
}

Vertex sumXZ(Vertex l, Vertex r)
{
	l.pos += r.pos;
	return l;
}
...
void ComputeUVs(Mesh &mesh)
{
	...
	// First loop thru all verts to find xz of main axis
	// Get Min Y and Max Y
	// Get Length in Y = MaxY - Min Y
	Vertex init; init.pos = Vec3(0.0f);
	Vertex sumPos = std::accumulate(mesh.vertexBuffer.begin(), mesh.vertexBuffer.end(), init, sumXZ);
	auto MaxY = std::max_element(mesh.vertexBuffer.begin(), mesh.vertexBuffer.end(), maxMinY);
	auto MinY = std::min_element(mesh.vertexBuffer.begin(), mesh.vertexBuffer.end(), maxMinY);

	sumPos.pos /= mesh.vertexBuffer.size();
	float min_Y = MinY->pos.y;
	float max_Y = MaxY->pos.y;
	float height = max_Y - min_Y;

	// loop thru all verts of mesh
	// v = vert.xz - point.xz
	// normalize(v)
	// x = v.x, z= v.z
	// y = v.y
	// The (x,y,z) w.r.t to the center of the cylinder

	Vertex point; // Center of cylinder
	point.pos = sumPos.pos; point.pos.y = (max_Y + min_Y) * 0.5f;

	for (unsigned int i = 0; i < mesh.vertexBuffer.size(); ++i)
	{
		// vector from center of cylinder to vertex
		Vec3 v_ector = mesh.vertexBuffer[i].pos - point.pos;

		// Project 3d vector down to 2d and normalize. Normalize becos we want cylinder with radius of 1
		float x_norm = v_ector.x / (std::sqrtf(std::powf(v_ector.x, 2.0f) + std::powf(v_ector.z, 2.0f)));

		// x = rcos(2PI*u); where r = 1, because we normalized to x_norm above
		float u = std::acosf(x_norm) / (2.0f * PI);

		// Get Y distance from min to vertex
		float height_frm_min = mesh.vertexBuffer[i].pos.y - min_Y;
		// y = h*v; where y is height_frm min and h is height(overall height)
		float v = height_frm_min / height;
