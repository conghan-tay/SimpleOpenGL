...
struct CubeFaceID
{
	...
	/*enum { FRONT, RIGHT, BOTTOM, LEFT, TOP, BACK, NUM_FACES };*/
};
...
void ComputeMirrorCamMats()
{
    if (mainCam.moved)
    {
        ...
        */
		...
		Vec3 view_dir = Normalize(-mirrorCamMirrorFrame);
		float near_plane = std::numeric_limits<float>::max();
		std::vector<Vec3> mid_sides = { Normalize(Vec3(0.5, 0.0, 0.0) - mirrorCamMirrorFrame),
										Normalize(Vec3(-0.5, 0.0, 0.0) - mirrorCamMirrorFrame),
										Normalize(Vec3(0.0, -0.5, 0.0) - mirrorCamMirrorFrame),
										Normalize(Vec3(0.0, 0.5, 0.0) - mirrorCamMirrorFrame) };

		
		for (int i = 0; i < mid_sides.size(); ++i)
		{
			float dist = Dot(mid_sides[i], view_dir);
			near_plane = (dist < near_plane) ? dist : near_plane;
		}

		Vec3 middle_near = near_plane * view_dir;
		Vec4 planes;
		planes.x = -2.0f *  glm::length(mid_sides[0] - middle_near);
		planes.y =  2.0f *glm::length(mid_sides[1] - middle_near);
		planes.z = -2.0f * glm::length(mid_sides[2] - middle_near);
		planes.w =  2.0f * glm::length(mid_sides[3] - middle_near);

		mirrorCamProjMat = Frustum(planes.x, planes.y, planes.z, planes.w, near_plane, std::numeric_limits<float>::max() * 0.5f);
    }
}
...
void ComputeSphereCamMats()
{
    ...
    */
	...
	GLenum types[6] = { GL_TEXTURE_CUBE_MAP_POSITIVE_Z , GL_TEXTURE_CUBE_MAP_POSITIVE_X,GL_TEXTURE_CUBE_MAP_NEGATIVE_Y ,
	...
	lookAt[CubeFaceID::LEFT] = spherePos + Vec3(-1.0f, 0.0f, 0.0f);
	lookAt[CubeFaceID::RIGHT] = spherePos + Vec3(1.0f, 0.0f, 0.0f);
	lookAt[CubeFaceID::TOP] = spherePos + Vec3(0.0f, 1.0f, 0.0f);
	lookAt[CubeFaceID::BOTTOM] = spherePos + Vec3(0.0f, -1.0f, 0.0f);
	lookAt[CubeFaceID::BACK] = spherePos + Vec3(0.0f, 0.0f, 1.0f);
	lookAt[CubeFaceID::FRONT] = spherePos + Vec3(0.0f, 0.0f, -1.0f);
	/*lookAt[1] = spherePos + Vec3(1.0f, 0.0f, 0.0f);
	lookAt[3] = spherePos + Vec3(-1.0f, 0.0f, 0.0f);
	lookAt[4] = spherePos + Vec3(0.0f, 1.0f, 0.0f);
	lookAt[2] = spherePos + Vec3(0.0f, -1.0f, 0.0f);
	lookAt[0] = spherePos + Vec3(0.0f, 0.0f, 1.0f);
	lookAt[5] = spherePos + Vec3(0.0f, 0.0f, -1.0f);*/
    /*  ... */
    ...
    */
	...
	upVec[CubeFaceID::RIGHT] = Vec3(0.0f, -1.0f, 0.0f);
	upVec[CubeFaceID::FRONT] =  Vec3(0.0f, -1.0f, 0.0f);
	upVec[CubeFaceID::TOP] = Vec3(0.0f, 0.0f, 1.0f);
	upVec[CubeFaceID::BOTTOM] =  Vec3(0.0f, 0.0f, -1.0f);
	upVec[CubeFaceID::BACK] =  Vec3(0.0f, -1.0f, 0.0f);
	upVec[CubeFaceID::LEFT] =  Vec3(0.0f, -1.0f, 0.0f);
	/*upVec[1] = Vec3(0.0f, -1.0f, 0.0f);
	upVec[3] = Vec3(0.0f, -1.0f, 0.0f);
	upVec[4] = Vec3(0.0f, 0.0f, 1.0f);
	upVec[2] = Vec3(0.0f, 0.0f, -1.0f);
	upVec[0] = Vec3(0.0f, -1.0f, 0.0f);
	upVec[5] = Vec3(0.0f, -1.0f, 0.0f);*/
    /*  ... */
    ...
    */
	sphereCamProjMat = Perspective(PI*0.5f, 1.0f, 0.01f, mainCam.farPlane);
    /*  ... */
}
...
void Bump2Normal(const unsigned char *bumpImg, unsigned char *normalImg, int width, int height)
{
    ...
    */
	...
	for (int j = 0; j < height ; ++j)
	{
		...
		{
			Vec3 B_u = Vec3(1.0f,0.0f,0.0f), B_v = Vec3(0.0f,1.0f,0.0f);
			
			if (i == 0)
			{
				// use i and i+ 1
				B_u.z = a * 0.5f * (float) (bumpImg[(i + 1) + j * width] - bumpImg[(i) + j * width]);
			}
			else if (i == width - 1)
			{
				// use width - 1 and width-2
				B_u.z = a * 0.5f * (float)(bumpImg[(width - 1) + j * width] - bumpImg[(width - 2)+j * width]);
			}
			else
			{
				// per normal
				B_u.z = a * (float)(bumpImg[(i + 1) + j * width] - bumpImg[(i - 1) + j * width]);
			}

			if (j == 0)
			{
				// use j and j + 1
				B_v.z = a * 0.5f * (float)(bumpImg[i + (j + 1) * width] - bumpImg[i + (j) * width]);
			}
			else if (j == height - 1)
			{
				// use height - 1 and height - 2
				B_v.z = a * 0.5f *(float)(bumpImg[i + (height - 1) * width] - bumpImg[i + (height - 2)* width]);
			}
			else
			{
				// per normal
				B_v.z = a * (float)(bumpImg[i + (j + 1) * width] - bumpImg[i + (j - 1) * width]);
			}

			B_u.z /= 255.0f;
			B_v.z /= 255.0f;

			Vec3 N = Cross(B_u, B_v);
			N = Normalize(N);

			normalImg[norm_counter] =(unsigned char) ( 127.5f * (N.x + 1.0f) );
			norm_counter += 1;
			normalImg[norm_counter] = (unsigned char) (127.5f * (N.y + 1.0f));
			norm_counter += 1;
			normalImg[norm_counter] = (unsigned char) (127.5f * (N.z + 1.0f));
			norm_counter += 1;
		}
	}
                        ...
                        int numComponents)
{
	...
	//int startY = verticalOffset * numComponents * imgWidth;
	//int startX = horizontalOffset * numComponents;
	int counter = 0;
	for (int j = verticalOffset; j < verticalOffset + size; ++j)
	{
		int currentY = j * numComponents * imgWidth;
		for (int i =horizontalOffset; i < horizontalOffset+ size; ++i)
		{
			int currentX = i * numComponents;
			
			destTex[counter++] = srcTex[currentX + currentY]; currentX += 1;
			destTex[counter++] = srcTex[currentX + currentY]; currentX += 1;
			destTex[counter++] = srcTex[currentX + currentY];
		}
	}

	std::vector<char> temp(size*size * numComponents);

	if (horizontalFlip)
	{
		for (int j = 0; j < size; ++j)
		{
			int y = j * numComponents * size;
			int flip_y = (size - 1 - j) * numComponents * size;
			for (int i = 0; i < size; ++i)
			{
				int x = i * numComponents;
				int c = x + y;

				int flip_c = x + flip_y;
				temp[flip_c] = destTex[c]; 
				flip_c += 1; c += 1;
				temp[flip_c] = destTex[c];
				flip_c += 1; c += 1;
				temp[flip_c] = destTex[c];
		...
		int total = size*size*numComponents;
	...
	std::vector<char> temp_2(size*size * numComponents);
	if (verticalFlip)
	{
		for (int j = 0; j < size; ++j)
		{
			...
			for (int i = 0; i < size; ++i)
			{
				int x = i * numComponents;
				int c = x + y;
				int flip_x = (size - 1 - i)*numComponents;
				int flip_c = flip_x + y;

				temp_2[flip_c] = destTex[c];
				flip_c += 1; c += 1;
				temp_2[flip_c] = destTex[c];
				flip_c += 1; c += 1;
				temp_2[flip_c] = destTex[c];
...
void SetUpSkyBoxTexture()
{
    ...
    /*  ... */
	CopySubTexture(cubeFace[0], cubeImgData, skyboxFaceSize, imgWidth, skyboxFaceSize, skyboxFaceSize, false, true,
		numComponents);
	CopySubTexture(cubeFace[1], cubeImgData, skyboxFaceSize, imgWidth, skyboxFaceSize, 2*skyboxFaceSize, false, true,
		numComponents);
	CopySubTexture(cubeFace[2], cubeImgData, skyboxFaceSize, imgWidth, 0, skyboxFaceSize, false, true,
		numComponents);
	CopySubTexture(cubeFace[3], cubeImgData, skyboxFaceSize, imgWidth, skyboxFaceSize, 0, false, true,
		numComponents);
	CopySubTexture(cubeFace[4], cubeImgData, skyboxFaceSize, imgWidth, 2*skyboxFaceSize, skyboxFaceSize, false, true,
		numComponents);
	CopySubTexture(cubeFace[5], cubeImgData, skyboxFaceSize, imgWidth, skyboxFaceSize, 3*skyboxFaceSize, false, true,
		numComponents);
	...
	{
		glTexImage2D(types[i], 0, internalFormat, skyboxFaceSize, skyboxFaceSize, 0, format,
			GL_UNSIGNED_BYTE, cubeFace[i]);
